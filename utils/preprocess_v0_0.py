...
class PreProcessing():
    ...
    def __init__(self, config, df_train_in, df_val_in):
        ...
        # In such a scenario, each visit should be treated as a 'different subject'
        ...
        cols_to_drop = ['ICGUID', 'Site', 'GT', 'DS_split', 'good_ori', 'DS_Phase', 'Wound Location', 'USorUK', 'Visit Number', 'orientation_deg',
        ...
        self.CAT_BIN_1 = [
            ...
            'limb_loss_history', 'limb_loss_study_foot_binary', 'angio_stent_bypass', 'infection_binary', 'recurrence', 'ckd', 'cirrhosis', 'DVT', 
            'heart_attack', 'stroke'
        ]
        ...
        self.ORDINAL = {
            ...
            'exu_volume': {
                'Light or small amount': 1,
                'Moderate amount': 2,
                'Heavy/large/copious amounts': 3
            }
        }
    ...
    def preprocess_df(self):
        ...
        # Step 0. Threshold "ulcer_duration" to upper bound (Q3 + 1.5 * IQR)
        ub = 27.1875
        X_train_num_1.loc[X_train_num_1['ulcer_duration'] > ub, 'ulcer_duration'] = ub
        X_val_num_1.loc[X_val_num_1['ulcer_duration'] > ub, 'ulcer_duration'] = ub
        #################################################
        # Process to calculate the upper bound
        ...
        # df = df[df['USorUK'] == 'US'].reset_index(drop = True)
        # df = df[df['DS_split'] == 'train'].reset_index(drop = True)
        # df = df[['subject_number', 'ulcer_duration']].drop_duplicates().reset_index(drop = True)
        # Q1 = df['ulcer_duration'].quantile(0.25)
        # Q3 = df['ulcer_duration'].quantile(0.75)
        # IQR = Q3 - Q1
        # ub = Q3 + 1.5 * IQR # -> 27.1875
        #################################################
        ...
        for col in self.NUMERICAL_3:
            ...
            if (col == 'first_dfu_age'):
                ...
                X_train_col = pd.DataFrame(self.df_train[['first_dfu_age', 'age', 'ulcer_duration']].apply(self.impute_first_dfu_age, axis = 1))
                X_val_col = pd.DataFrame(self.df_val[['first_dfu_age', 'age', 'ulcer_duration']].apply(self.impute_first_dfu_age, axis = 1))
    ...
    def impute_first_dfu_age(self, x):
        ...
        if (str(x[0]) == 'nan') or (x[0] > x[1]):
            # if (a) "first_dfu_age" is NaN; or (b) "first_dfu_age" is greater than "age"
            # impute with patient's current age - ulcer duration (months / 12)
            return x[1] - (x[2] / 12)  
        else:
            return x[0]