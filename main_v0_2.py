...
Difference compared to main_v0_1.py:
    - Excluded small ulcers (< 0.52 cm^2) from training dataset
...
from utils.preprocess_v0_1 import PreProcessing
...
def main(config, seed = 1337):
    ...
    # Avoid overwriting existing training history
    ...
    if (not config.IMAGE_PREPROCESS['INCLUDE_EU']): # Keep only US subjects
        df = df[df['USorUK'] == 'US'].reset_index(drop = True)
        print(f"    >> EU subjects dropped ({df.shape[0]} rows remaining)")
        
    # Exclude small ulcers (< 0.52 cm^2) from training data
    df = df[df['cm2_planar_area'] > 0.52].reset_index(drop = True)
    
    # Separate data into training/test sets
    df_train = df[~df['subject_number'].str.strip().isin(config.TEST_SET)].reset_index(drop = True)
...
def sanity_check_feature_process(config, df_train, df_val, df_train_transformed_in, df_val_transformed_in):
    ...
    # Threshold value of feature "ulcer_duration"
    ub = 27.1875
    df_train_cp.loc[df_train_cp['ulcer_duration'] > ub, 'ulcer_duration'] = ub
    df_val_cp.loc[df_val_cp['ulcer_duration'] > ub, 'ulcer_duration'] = ub
    ...
    for df in [df_train_cp, df_val_cp]:
        ...
        for col in df.columns:
            ...
            for idx in range(len(df)):
                ...
                ##############################################################
                if (col in preprocess.NUMERICAL_1) or (col in preprocess.NUMERICAL_2):
                    
                    ref = df_train_transformed.loc[idx, col] if is_train else df_val_transformed.loc[idx, col]
                    
                    if (col == 'ulcer_duration') and (res > ub): res = ub # Threshold "ulcer_duration" value
                ...
                if (col in preprocess.NUMERICAL_3):
                    
                    if (col == 'first_dfu_age'):
                        
                        res_age = df.loc[idx, 'age']
                        
                        ref = df_train_transformed.loc[idx, col] if is_train else df_val_transformed.loc[idx, col]
                        age = df_train_transformed.loc[idx, 'age'] if is_train else df_val_transformed.loc[idx, 'age']
                        
                        if (np.isnan(res)) or (res > res_age):
                            ref = ref * numerical_stats[f'train_std_{col}'] + numerical_stats[f'train_mean_{col}']
                            age = age * numerical_stats[f'train_std_age'] + numerical_stats[f'train_mean_age']
                            print_str = f"Imputed feature {col} for subjectID {subjectID} - {visit}: {ref:.3f} (subject current age: {age})"
                            if (is_train):
                                print(f"    >> TRAIN SET - {print_str}")
                            else:
                                print(f"    >> VAL SET - {print_str}")
