...
if __name__ == '__main__':
    ...
    with pd.ExcelWriter(f"{out_excel_path}/{'_'.join(prediction_path.split('/')[-2:])}_hs_{hs}.xlsx", engine = 'openpyxl') as writer:
        ...
        # worksheet.column_dimensions['group'].width = 20
        # worksheet.column_dimensions['acc'].width = 10
        # worksheet.column_dimensions['sen'].width = 10
        # worksheet.column_dimensions['spe'].width = 10
        # Assuming 'worksheet' is your active worksheet object
        ...
        # # Set print options to scale the sheet size to fit all columns on one page width
        # worksheet.page_setup.fitToWidth = 1  # Fit all columns to one page width
        # worksheet.page_setup.fitToHeight = 0  # Unlimited height
        # worksheet.page_setup.orientation = worksheet.ORIENTATION_LANDSCAPE  # Change orientation to landscape for better fitting